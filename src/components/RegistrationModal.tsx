...
export const RegistrationModal: React.FC<RegistrationModalProps> = ({ isOpen, onClose, items, onSuccess }) => {
  ...
  const handleApplyCoupon = () => {
    ...
    setTimeout(() => {
      const validCodes = ['data_and_ai', 'emek_hefer', 'halhatz', 'shavot', 'emek hefer', 'data and ai', 'police', 'lion', 'cyber'];
          ...
          <div className="mb-8">
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="flex-grow px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
                disabled={isCouponValid || isApplyingCoupon}
              />
              <button 
                onClick={handleApplyCoupon}
                disabled={isCouponValid || isApplyingCoupon || !coupon}
                className="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-colors disabled:opacity-50 flex items-center gap-2"
              >
                {isApplyingCoupon ? <Loader2 className="w-4 h-4 animate-spin" /> : 'החל קופון'}
              </button>
            </div>
            {couponError && <p className="text-red-500 text-xs mt-1">{couponError}</p>}
            {isCouponValid && <p className="text-emerald-600 text-xs mt-1 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> הקופון הוחל בהצלחה!</p>}
          </div>
